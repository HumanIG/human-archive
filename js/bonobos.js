$(document).ready(function() {
	...
	if(window.mobilecheck() === false) {
		...
		$('#laptop-img').attr('src', 'img/laptop-bonobos.png');
	}
});

function placeLaptopSite() {
	var $stamp = $('#bonobos-stamp');
	var $laptop_img = $('#laptop-img');
	var laptop_img2 = $('#laptop-img');
	var $in_laptop = $('#in-laptop');
	var laptopOffset = laptop_img2.offset();
	...
	$stamp.css({"width":$laptop_img.width()/6+"px","top":laptopOffset.top-($stamp.height()/2)+20,"left":laptopOffset.left+(width/2)+($stamp.width()/2)+90});
	setTimeout(function(){
		$stamp.fadeIn();
	}, 200);
...
function placeAddressBar() {
	...
	// These values were found through testing on the vector laptop image.
	var left_ratio = 0.2157684867075665;
	var top_ratio = 0.0449952244165172;
	var width_ratio = 0.5686858309990664;
	var height_ratio = 0.1374378363228699;

	if(window.mobilecheck() === true && $(window).width() > 700) {
		width_ratio = 0.5706858309990664;
	} else if (window.mobilecheck() === true) {
		left_ratio = 0.2167984867075665;
		width_ratio = 0.5709858309990664;
	}
